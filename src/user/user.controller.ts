...
// Create User
  ...
  async updateUserById(@Param('id') id: ObjectId, @Body() user: UpdateUserDto) {
    return await this.userService.findByIdAndUpdate(id,user).then((promise) => promise).catch((error) => error);
  }

   @Delete('/remove/:id')
  async removeUserById(@Param('id') id: ObjectId) {
    return await this.userService.findByIdAndRemove(id,).then((promise) => promise).catch((error) => error);
  }
